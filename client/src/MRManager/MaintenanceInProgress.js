...
export default function MaintenanceInProgress() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter only maintenance records that are in progress
                const inProgressMaintenanceRecords = response.data.filter(req => req.status === 'In Progress');
                setAllRecords(inProgressMaintenanceRecords);
                setDisplayRecords(inProgressMaintenanceRecords);
                setLoading(false);
            } catch (err) {
    ...
    return (
        <div className="view-status-container">
            <h1 style={{ color: "white" }}>Maintenance In Progress</h1>
