...
const RepairRequests = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await axios.get('http://localhost:8070/manager/repair');
        const pendingRequests = response.data.filter(req => req.status === 'Pending');
        setRepairRequests(pendingRequests);
  ...
  }, [searchTerm, repairRequests]);

  const handleApproveReject = async (id, status) => {
    try {
      await axios.put(`http://localhost:8070/manager/repair/${id}/approve`, { status });
      const updatedRequests = repairRequests.filter(req => req._id !== id);
  ...
  const handleAssignTechnician = async () => {
    try {
      await axios.put(`http://localhost:8070/manager/repair/${selectedRequest}/assign`, { tech_id: techId });
  ...
  return (
    <div className="container">
      <h1 style={{color: "white"}}>View Repair Requests</h1>
      <input
        type="text"
