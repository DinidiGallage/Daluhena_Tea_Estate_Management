...
export default function MaintenancePending() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter only maintenance records that are pending
                const pendingMaintenanceRecords = response.data.filter(req => req.status === 'Pending');
                setAllRecords(pendingMaintenanceRecords);
                setDisplayRecords(pendingMaintenanceRecords);
                setLoading(false);
            } catch (err) {
    ...
    return (
        <div className="view-status-container">
            <h1 style={{ color: "white" }}>Maintenance Pending Requests</h1>
