...
export default function MaintenanceStatus() {
    ...
    useEffect(() => {
        let filteredRecords = allRecords.filter(record => {
            return (record.item_id.toString().includes(searchItemId) &&
                    ...
                    (record.status.toLowerCase().includes(searchStatus.toLowerCase())))
        });
    ...
    return (
        <div className="view-status-container">
            <h1 style={{ color: "white" }}>Maintenance Request Status</h1>
                ...
                <tbody>
                    {displayRecords.map((record, index) => (
                        <tr key={index}>
                            ...
                            <td className={
                                record.tech_id ? 'detail-assigned' : 'detail-pending'}>
