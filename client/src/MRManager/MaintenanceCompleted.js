...
export default function MaintenanceCompleted() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter only maintenance records that are Completed
                const CompletedMaintenanceRecords = response.data.filter(req => req.status === 'Completed');
                setAllRecords(CompletedMaintenanceRecords);
                setDisplayRecords(CompletedMaintenanceRecords);
                setLoading(false);
            } catch (err) {
    ...
    const deleteRecord = async (recordId) => {
        try {
            await axios.delete(`http://localhost:8070/maintenance/delete/${recordId}`);
    ...
    return (
        <div className="view-status-container">
            <h1 style={{ color: "white" }}>Maintenance Completed</h1>
                ...
                <tbody>
                    {displayRecords.map((record, index) => (
                        <tr key={index}>
                            ...
                            <td className={record.status === "Pending" ? 'detail-pending' :record.status === "Completed" ? 'detail-Completed-assigned' :'detail-completed'}>{record.tech_id ? record.tech_id : 'Not Assigned'}</td>
                            <td className={record.status === "Pending" ? 'detail-pending' :record.status === "Completed" ? 'detail-Completed-assigned' :'detail-completed'}>{record.start_date ? new Date(record.start_date).toLocaleDateString() : 'Not Started'}</td>
